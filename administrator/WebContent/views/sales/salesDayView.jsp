...
<body>
		...
		<div class="page-contentbar">
			...
			<div id="page-right-content">
				...
				<div class="container">
					...
					<div class="row" id="Day_Sales">
						<div class="col-sm-12">
						<h4 class="m-t-0 header-title"><%= startDate %> ~ <%= endDate %> || 일별 매출 내역</h4>
						 <div class="table-responsive m-b-20">
						 <table id="datatable-buttons" name="day" class="table table-striped table-bordered">
                                        <thead>
                                        <tr>
                                        	<th>캠핑장명</th>
                                            <th>날짜</th>
                                       ...
                                       <tr id="detailTr">
                                       <input type="hidden" value="<%= cd.getsCampCode() %>">
		                                       <td><%= cd.getsCampName() %></td>
		                                       <td><%= cd.getsDay() %></td>
		                                       <td><%= formatter.format(cd.getsCost()) %></td>
		                                       <td><%= formatter.format(Math.round(cd.getsCost()*0.15)) %></td>
		                                       <td><%= formatter.format(Math.round(cd.getsCost() * 0.85)) %></td>
		                                       <td><%= cd.getsCount() %></td>
			...
			<script src="https://canvasjs.com/assets/script/canvasjs.min.js"></script>	
			 ...
			 <div style="width: 41%; height: 370px; float: right;">
			          ...
			          <div class="card-box" style="width: 100%; height: 50%; margin: 0;">
                       	...
                       	<h4 class="m-t-0 text-dark">건당 평균 순이익</h4>
			         ...
			         <div class="card-box" style="width: 100%; height: 50%; margin: 0;">
                       	...
                       	<h4 class="m-t-0 text-dark">건당 평균 수수료</h4>
	...
	$(function(){	
	 ...
	 function dayDetail(value){
		  ...
		  $title.text(year + "년 " + month + "월 " +day +"일 " + campName + "의 상세매출");
		  
		  <%-- var nDate = "<%= csList.get(0).getsPayDate() %>" ; --%>
		  	  		  
			<% for(CampSales cs : csList){ %>
		  		
		  	if((<%= cs.getsCampCode() %> == code) && ("<%= cs.getsPayDate().substring(0, 10) %>" == fullday)){
		  		
		  		$tr = $("<tr>");
		  		$td0 = $("<td>").text("<%= cs.getsPayDate() %>");
		  		$td1 = $("<td>").text("<%= cs.getsUserName() %>");
		  		$td2 = $("<td>").text("<%= cs.getsReDate() %>");
		  		$td3 = $("<td>").text("<%= cs.getsPayType() %>");
		  		$td4 = $("<td>").text("<%= formatter.format(cs.getsCost()) %>");
		  		$td5 = $("<td>").text("<%= formatter.format(Math.round(cs.getsCost()*0.15)) %>");
		  		$td6 = $("<td>").text("<%= formatter.format(Math.round(cs.getsCost()*0.85)) %>");
		 ...
		 var chart = new CanvasJS.Chart("chartContainer", {
			...
			$("#averageview").text("건당 평균매출액 : " + finalMonthEarn);
			
			$("#MonthEa").text(finalMonthProfit);
			$("#MonthAv").text("건당 평균매출액 : " + finalMonthEarn); 
