...
<div id="page-wrapper">
                        ...
                        <div class="row" id="detail_box">
                            <div class="col-sm-12"> 
                                ...
                                <ul id="detail">
      							...
      							<button class="btn btn-primary btn-xs" type="button" onclick="approval(this);">승인하기</button>
		...
		function update(value){
			...
			<% for(CampInfo ca : cList) {%>
				...
				if(cNo == (<%= ca.getcCode() %> +"")){
					...
					var $li0 = $("<input id='Code'>").val("<%= ca.getcCode() %>").attr("type", "hidden");
					var $li1 = $("<li>").text("캠핑장명 : <%= ca.getcName() %>");
					var $li2 = $("<li>").text("주소 : <%= ca.getcAddress() %>");
					var $li3 = $("<li>").text("전화번호 : <%= ca.getcPhone() %>");
					var $li4 = $("<li>").text("홈페이지 주소 : <%= ca.getcUrl() %>");
					var $li5 = $("<li>").text("사업장 사진");
					var $br = $("<br>");
					$li5.append($br);
					var $li6 = $("<li>").text("테마(입지조건) : <%= ca.getcTheme() %>");
					var $li7 = $("<li>").text("사이트 옵션 정보 : <%= ca.getcOption() %>");
					var $li8 = $("<li>").text("예약시작 가능일자 : <%= ca.getcAvailableDate() %>");
					var $li9 = $("<li>").text("포스팅 서비스 제휴 여부 : <%= ca.getcPosting() %>");
					var $li10 = $("<li>").text("환불 규정 : <%= ca.getcRefundment() %>");
					var $li11 = $("<li>").text("제안/기타 : <%= ca.getcEtc() %>");
		...
		function approval(value){
			...
			location.href = "<%= request.getContextPath() %>/update.ca?cNo="+cNo;
